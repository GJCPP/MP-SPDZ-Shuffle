...
namespace myShuffle {
    ...
    */
    ...
    protected:
        ...
        // Protocol interaction depth. Unlike Player::CommStats::rounds, this
        // counts one logical exchange regardless of payload chunking.
        ...
        void add_logical_rounds(size_t rounds = 1);
    ...
    public:
        ...
        */
        ...
        void broadcast_pairs(std::vector<std::array<ClearType, 2>>& values);
        ...
        */
        ...
        void commit_and_open(const T& message, std::vector<T>& open_msg) {
            ...
            // Commit and reveal are two sequential broadcast rounds.
        ...
        */
        ...
        // Raw transport-operation count, used only to detect MP-SPDZ MAC-check
        // broadcasts. Benchmark output uses count_total_rounds().
    ...
    inline void mpc_comm::broadcast_pairs(
            std::vector<std::array<ClearType, 2>>& values)
    {
        if (values.size() != static_cast<size_t>(n_party)) {
            throw std::runtime_error("broadcast_pairs: invalid party count");
        }
        std::vector<octetStream> buff(n_party);
        if (my_number != 0) {
            buff[my_number].store(values[my_number][0]);
            buff[my_number].store(values[my_number][1]);
        }
        P.Broadcast_Receive(buff);
        for (int party = 1; party != n_party; ++party) {
            buff[party].get(values[party][0]);
            buff[party].get(values[party][1]);
        }
        add_logical_rounds();
    }
